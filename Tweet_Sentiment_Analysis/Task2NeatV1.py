...
############## 7 ################  add in options for changing the meaning of a tweet and amplifying the meaning of a tweet
changeMeaning = ["not isn't no can't cannot shouldn't wouldn't nay never couldn't"]


changeMeaning = TokeniseInput(changeMeaning)

amplifyMeaning = ["very really incredibly insanely extremely exceedingly \
                  exceptionally especially tremendously immensely vastly hugely\
                  genuinely truly honestly greatly considerably substantially \
                  appreciably significantly markedly sizeably seriously \
                  materially signally profoundly so"]

#amplifyMeaning = ["very really incredibly extremely honestly greatly so"]

amplifyMeaning = TokeniseInput(amplifyMeaning)
...
############### 8 ############### Need to redefine the tokenising function to include this addition
"""
Here, we can play around with what things to take out and what to leave in. 
Some commented out appear to negatively impact performance.
"""
print('Making a tokenising function')
def CleanStrings(string):
    """
...
#    string = (string).replace('/', ' ')
    ...
    string = (string).replace('{', '')
    string = (string).replace('}', '')
    for i in changeMeaning:                          # Can join words together so 'not sunny' becomes 'notsunny'
        string = (string).replace(i+' ', i + ' ' + i)
#    for i in amplifyMeaning:                        # Can join words together so 'very good' becomes 'verygood'
#       string = (string).replace(i+' ', i + ' ' + i) 
    string = string.lower()
#    string = re.sub(r'[0-9]+', '', string)
    return string
...
############### 9 ############### now make lists of good and bad words
...
################ 11 ############### make a dictionary of words that arent stopwords
"""
Stopwords appeared to have a negative effect on the accuracy so this has been commented out
"""
...
#################### 12 ################ Include N-Grams
"""
Want to add in N-Grams... (Just for N=2)
"""
n = 2
good2Grams = []
bad2Grams = []
for tweet in goodTweetsTrain[:,1]:
    twoGrams = ngrams(tweet.split(), n)
    for grams in twoGrams:
        good2Grams.append(grams)
for tweet in badTweetsTrain[:,1]:
    twoGrams = ngrams(tweet.split(), n)
    for grams in twoGrams:
        bad2Grams.append(grams)

goodWords = goodWords + good2Grams
badWords = badWords + bad2Grams
...
############### 13 ################# Find frequency of each word (or tuple of words)     
"""
...
############### 14 ################# remove the words with low freqencies
...
############ 15 ############ make a dictionary of probabilities
"""
...
############## 16 ############## give a prediction for a tweet
"""
...
############## 17 ############## now see the accuracy on our test set
"""
